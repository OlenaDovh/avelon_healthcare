...
Returns:
    ...
    analyses = filtered_analyses_queryset(what_to_check=what_to_check, disease=disease, for_whom=for_whom, biomaterial=biomaterial)
    cart = get_cart(request)
    filters_data = analysis_filter_values()
    return render(request, 'avelon_healthcare/analysis/pages/analysis_list.html', {'analyses': analyses, 'cart': cart, **filters_data, 'selected_what_to_check': what_to_check, 'selected_disease': disease, 'selected_for_whom': for_whom, 'selected_biomaterial': biomaterial})
...
Returns:
    ...
    return render(request, 'avelon_healthcare/analysis/pages/cart_detail.html', {'analyses': analyses, 'total_price': total_price, 'recommended_analyses': recommended_analyses, 'order_form': order_form})
