...
Відповідає за поведінку, описану в цьому компоненті застосунку."""
    ...
    payment_method = forms.ChoiceField(choices=PaymentMethod.choices, widget=forms.Select(attrs={'class': 'form-select'}))
    analyses = forms.ModelMultipleChoiceField(queryset=Analysis.objects.filter(is_active=True).order_by('name'), widget=forms.CheckboxSelectMultiple)
...
Returns:
    Результат виконання операції."""
        ...
        if not user:
            required = [cleaned_data.get('last_name'), cleaned_data.get('first_name'), cleaned_data.get('phone'), cleaned_data.get('email')]
            if not all(required):
                raise forms.ValidationError('Оберіть користувача або заповніть всі дані нового пацієнта.')
        return cleaned_data
