...
Returns:
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            order: Order = Order.objects.create(user=user, last_name=last_name, first_name=first_name, middle_name=middle_name, phone=phone, email=email, total_price=total_price, payment_method=payment_method, status=OrderStatus.NEW)
            OrderItem.objects.bulk_create([OrderItem(order=order, analysis=a, price=a.price) for a in analyses])
            return redirect('orders:support_order_list')
    else:
        form = SupportOrderCreateForm()
    return render(request, 'avelon_healthcare/orders/pages/support_order_form.html', {'form': form})
...
Returns:
    ...
    return render(request, 'avelon_healthcare/orders/pages/support_order_update.html', {'form': form, 'order': order})
