...
Returns:
    ...
    order = create_order_from_analyses(analyses=analyses, payment_method=payment_method, user=user, last_name=last_name, first_name=first_name, middle_name=middle_name, phone=phone, email=email)
    transaction.on_commit(lambda: send_order_email_task.delay(order.id))
    _clear_cart(request)
