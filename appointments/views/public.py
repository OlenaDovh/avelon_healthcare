    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if request.user.is_authenticated:
                appointment = fill_appointment_from_user(appointment=appointment, user=request.user)
            else:
                appointment = fill_appointment_from_guest_data(appointment=appointment, cleaned_data=form.cleaned_data)
        ...
        form = form_class()
    return render(request, 'avelon_healthcare/appointments/pages/appointment_form.html', {'form': form, 'directions': Direction.objects.annotate(doctors_count=Count('doctors')).filter(doctors_count__gt=0).order_by('name')})
