...
Returns:
    ...
    users_data = [{'id': user.id, 'full_name': user.full_name or user.username, 'phone': getattr(user, 'phone', '') or '—', 'email': user.email or '—'} for user in patient_users_queryset().order_by('last_name', 'first_name')]
    return render(request, 'avelon_healthcare/appointments/pages/support_appointment_form.html', {'form': form, 'users_data': users_data})
...
Returns:
    ...
    return render(request, 'avelon_healthcare/appointments/pages/support_appointment_update.html', {'form': form, 'appointment': appointment})
