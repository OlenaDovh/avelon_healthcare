...
class SupportAppointmentCreateForm(AppointmentCreateForm):
    ...
    middle_name = forms.CharField(required=False, label='По батькові', max_length=150, widget=forms.TextInput(attrs={'class': 'form-control'}))
    phone = forms.CharField(required=False, label='Телефон', max_length=30, widget=forms.TextInput(attrs={'class': 'form-control'}))
...
Returns:
    Any: Результат виконання."""
        ...
        if user and guest_fields_filled:
            raise ValidationError('Оберіть або зареєстрованого користувача, або заповніть дані незареєстрованого пацієнта.')
        if not user and (not guest_fields_filled):
            raise ValidationError('Оберіть зареєстрованого користувача або заповніть дані незареєстрованого пацієнта.')
        if not user:
            if not all([last_name, first_name, phone, email]):
                raise ValidationError('Для незареєстрованого пацієнта потрібно заповнити прізвище, імʼя, телефон і email.')
